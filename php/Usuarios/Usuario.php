...
class Usuario
{
    ...
    public function filtarPartidas($filtro) {
        try {
            ...
            $datosFiltrado["limite"] = $filtro["limite"]; //Si se lo asigno no devuelve ningun valor pero tampoco da error
            //Instancio la BD
            $bd = new bd();
            //Envio la petición
            ...
            if(!is_nan(intval($numPag))){
                $numPag = ceil($numPag / $filtro["limite"]);
            }
     ...
     */
    public function comprobarFechas($fecha1, $fecha2) {
     ...
     */
    public function devolverFechasPostDiaActual($fecha) {
